...
One-time install that makes the kit available in every VS Code project automatically.
...
Copy the kit directly into your project:

```bash
# Copy into .github (recommended for teams)
cp -r agents instructions prompts skills templates tools memory .github/

# Or copy to workspace root
cp -r agents instructions prompts skills templates tools memory ./
```
...
All tools require only Python 3.8+ stdlib — no pip install needed (except `global_memory.py` which needs `pip install duckdb`).
...
python tools/global_memory.py search --query "type hints" --format markdown
```

See [tools/README.md](tools/README.md) and [instructions/memory-context.instructions.md](instructions/memory-context.instructions.md) for full documentation.
...
| 11 consolidated instructions (down from 30) | ~75% context token reduction, scoped loading |
